...
#isosceles triangle
def triangle_isos(export_file_txt,x_y_ratio, outer_length,inner_length, fil_width, gap_to_fill):
    with open(export_file_txt, "w") as f:
        ...
        while length > inner_length:
            length -= fil_width
            x = length / 2
            y = x * slope + gap_to_fill*gap_count
            f.write('\n\rG1 X' + str(x) + ' Y' + str(y))
...
if __name__ == '__main__':
    ...
    outer_length_horizontal = 10
    inner_length_horizontal = 5
    fil_width = 0.75
    x_y_ratio = 15 # = y/x
    gap_to_fill = -2 # use negative value for larger gap and overextrusion; use + value for smaller gap and underextrusion
    gradient_fraction = 1/2
    triangle_isos(export_file_txt, x_y_ratio, outer_length_horizontal, inner_length_horizontal, fil_width, gap_to_fill)
    ...
    print(input_line_list[1])

    segments = ['length', 1] # ['type', value], type options: 'length', 'number'

    pressure_range = [10,20] #[nodes, strut]
    ### Pressure box and valve settings
    com = ["serialPort1"]
    valve = [6, 7]

    setpress_start = str('\n\r{preset}' + com[0] + '.write(' + str(setpress(pressure_range[1])) + ')')  # material 1

    toggleON = str('\n\r{preset}' + com[0] + '.write(' + str(togglepress()) + ')')  # turn on material 2
    toggleOFF = toggleON
    ...
    with open(export_gradient_file_txt, "w") as f:
        ...
        for i in range(len(input_line_list[0])):
            if i > 3 and 0 < count < 3:
                print('---------------------new line', [input_line_list[0][i], input_line_list[1][i]])
                ...
                count += 1
                pressure_range = [pressure_range[1], pressure_range[0]]
                if count == 3:
                    count = 0
            elif i > 0 or count == 0:
                print('---------------------NO GRADIENT line', [input_line_list[0][i], input_line_list[1][i]])
                line = '\nG1'
                for j in range(len(input_line_list[0][i])):
                    line += ' '+ input_line_list[0][i][j] + str(input_line_list[1][i][j])
