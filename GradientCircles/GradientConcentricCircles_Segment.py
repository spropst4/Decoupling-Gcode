...
def segmentCircleG3(input_line, section_arc_length, pressure_range):
    global pressure
    ## find arc_angle
    ...
    pressure_incr = (pressure_range[1] - pressure_range[0])/(0.5*num_sections)

    x_prev = r
    y_prev = 0
    theta_for_ij = 0
    theta_for_xy = theta_between_moves
    prev_pressure = pressure_range[0]
    for c in range(num_sections):
        ...
        if pressure < prev_pressure:
            f.write(valveOFF)
        f.write(pressurebox_setpress_str_command(port, pressure, False))
        if pressure < prev_pressure:
            f.write(valveON)
...
if __name__ == '__main__':
    gcode_txt_input = 'Circles_gcode.txt'
    export_file = 'TestCircles_Gradient_gcode.txt'
...
valve = 6
port = 'serialPort1'
pressure_range = [22, 39]
arc_segments = 1

set_pressure = pressurebox_setpress_str_command(port, pressure_range[0], True)
toggle_start = pressurebox_toggle_str_command(port, True)
toggle_end = pressurebox_toggle_str_command(port, False)
valveON = valve_str_command(valve, 1)
valveOFF = valve_str_command(valve, 0)

f = open(export_file, 'w')
f.write('\nG91')
f.write(set_pressure)
f.write(toggle_start)
f.write(valveON)
f.write('\nG1 X2')
...
for i in range(len(input_line[0])):
    ...
    if G_list == 3 or G_list == 2:
        ...
        segmentCircleG3(input_line_current, arc_segments, pressure_range)
    elif var_line != []:
        pressure_range = [pressure_range[0], pressure_range[1]]
        count +=1
