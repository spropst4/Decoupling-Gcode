...
accel = 700 # mm/s^2 #Hyrel is set at 700
decel = -700 # mm/s^2


'''Offset notes:
- 1-2 hour old SE1700, 23 psi, F10, A700, switching Nozzle, offset = 3.3 mm
offset = 3.7 mm was working well
'''
offset = 3.7#3.3 # #use a negative number to increase length of time/material being on (units in mm)
offset_initial = 4.8# (units in mm)
start_delay = 0 # (units in mm)

gcode_txt_imported ='image2gcode_cube.txt'#"1_Generate_Checkerboard_SwitchingNozzle_gcode.txt"#'image2gcode_cube_checkerboard.txt' #"1_Generate_Checkerboard_SwitchingNozzle_gcode.txt" #"1DGC_Generate_Checkerboard_SwitchingNozzle_gcode.txt" #"1DGC_Generate_Checkerboard_M1.txt"
final_gcode_txt_export = 'image2gcode_cube_HYREL.txt' #"1_Generate_Checkerboard_SwitchingNozzle_HYREL.txt"#'image2gcode_cube_HYREL.txt' #"1_Generate_Checkerboard_SwitchingNozzle_HYREL.txt"
...
PING_delay = 0.0008811907853908882 #0.011305268287658692 # from AEROTECH_PING_data.txt
...
port_aerotech = 1 # port named in python code to connect to hyrel
port_python = 2 #this port is defined in hyrel to connect to python
...
def generate_gcode(final_gcode_txt_export, accel, Z_var, z_o, feed, Sum_G_command_dict, Sum_var_dict, Sum_coord_dict, resync_trigger_distance_dict):
    ...
    with open(final_gcode_txt_export, "w") as f:
        ...
        f.write('\nG90')
        f.write("\nG0 " + Z_var + str(z_o) + "\n")
        f.write("G1 F" + str(feed*60) + "\n\r") #hyrel speed is in mm/min

        f.write('\n\rM792 ;SEND Start')
        f.write('\nG4 P3000\n\r') #hyrel pause is in ms
...
print("\nTotal time to translate GCode to timestamps: ", total_time)
