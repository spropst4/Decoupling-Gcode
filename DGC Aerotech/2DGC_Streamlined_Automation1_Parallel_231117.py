...
path = 'C:\\Users\\sprop\\PycharmProjects\\pythonProject\\'
file_folder = 'ParallelPrints_Test' #either a single txt file or a folder containing all g-code txt files (for parallel printing)

#export_path = 'C:\\Users\\sprop\\PycharmProjects\\pythonProject\\DGC_Aerotech_gcode'
...
def final_commands_time(time_dict):
    command_list = []
    ...
    for i in range(len(time_dict)):
        ...
        if type(entry) != str:
            ...
            if initial_commands_trigger == 0:
                initial_commands_list = command_list
            else:
    ...
    return time_based_dict_final, command_dict_final, initial_commands_list
...
for file in f_list_final:
    ...
    ## creates final dictionaries of commands and times to use
    final_output = final_commands_time(time_dict)
    time_dict_final_current = final_output[0]
    command_dict_final_current = final_output[1]
    initial_commands_list_current = final_output[2]
    print(file, 'number of commands', len(time_dict_final_current))
    for i in range(len(time_dict_final_current)):
        command = command_dict_final_current[i]
        key = time_dict_final_current[i]
    ...
    if file == f_list_final[0]:
        print(list(compiled_command_dict.keys()))
...
print('number of compiled commands', len(time_dict_final_current))
