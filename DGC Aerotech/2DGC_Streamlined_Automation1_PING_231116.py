def openport(port):
    ...
    return serial.Serial("COM" + str(port), baudrate=baudrate, bytesize=bytesize, timeout=timeout,
...
offset = 2  # 0.45 #0.5 #2.92 #3.75 #2.5 #use a negative number to increase length of time/material being on (units in mm)
offset_initial = offset  # 5 #3.5
start_delay = 0  # (units in mm)

gcode_txt_imported = '231116_DGC_Checkerboard_SwitchingNozzle_gcode.txt'  # "1DGC_Generate_Gradient_M1_gcode.txt" #"1DGC_Generate_Checkerboard_SwitchingNozzle_gcode.txt" #"1DGC_Generate_Checkerboard_M1.txt"
final_gcode_txt_export = gcode_txt_imported.replace('gcode',
                                                    'aerotech_test')  # '230831_1mm_32_70_Gradient_diamond_lattice_aerotech.txt'
...
num_initial_pressureSet = 2  # (aka number of materials used)
...
def condense_gcode(resync_number, resync_type, distance_commands_dict, G_command_dict, All_var_dict, X_dist_dict,
                   Y_dist_dict, Z_dist_dict, I_dist_dict, J_dist_dict):
    ## pythag thm
    def pythag(x, y, z):
        return (sqrt(abs(float(x)) ** 2 + abs(float(y)) ** 2 + abs(float(z)) ** 2))
    ...
    for j in range(len(distance_commands_dict)):
        ...
        if type_check != str and i < len(G_command_dict) - 1:
            ...
            if G_command_dict[i] == G_command_dict[i - 1] and All_var_dict[i] == All_var_dict[i - 1] and slope_dict[
                i] == slope_dict[i - 1]:
                # '''G1 Commands'''
...
def generate_gcode(final_gcode_txt_export, accel, Z_var, z_o, feed, Sum_G_command_dict, Sum_var_dict, Sum_coord_dict,
    ...
    with open(final_gcode_txt_export, "w") as f:
        with open(intro_gcode, "r") as g:
            for line in g:
                line = line.replace('{Z_var}', Z_var).replace('{ramprate}', str(accel)).replace('{feed}',
                                                                                                str(feed)).replace(
                    '{Z_start}', str(Z_start))
                f.write(line)
            g.close()
...
generate_gcode(final_gcode_txt_export, accel, Z_var, Z_start, feed, Sum_G_command_dict, Sum_var_dict, Sum_coord_dict,
               resync_trigger_distance_dict)
...
offset_time_initial = float(
    distance2time(accel_profile_distance, accel_profile_time, feed, accel, decel, [abs(offset_initial)])[0])
...
set_press = '[%s]' % ', '.join(map(str, initial_commands[:num_initial_pressureSet]))
initial_toggle = '[%s]' % ', '.join(map(str, initial_commands[num_initial_pressureSet:]))
...
def thread_exec_commands():
    global i
    global command_dict_final
    global count
    global offset
    global resync_test_time_adjust
    global got_a_ping
    global add_time
    global resync_type
    global start_time

    print(i, len(command_dict_final), got_a_ping)
    while i < len(command_dict_final):
        ...
        if real_time >= current_time_stamp_execute:
            print(real_time, current_time_stamp_execute)
...
if __name__ == '__main__':
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        with conn:
            ...
            list_data = []
            #conn.setblocking(False) # sets it to non-blocking
            ...
            while i < len(command_dict_final):
                ...
                current_time_stamp_execute = (time_based_dict_final[i] - offset) + add_time
                if real_time >= current_time_stamp_execute:
                    print('----executed command number '+ str(i+1) + ' of '+str(len(command_dict_final)) )
                    exec(command_dict_final[i])
                    i += 1
