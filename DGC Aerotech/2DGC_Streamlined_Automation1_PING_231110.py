...
offset = 4#0.45 #0.5 #2.92 #3.75 #2.5 #use a negative number to increase length of time/material being on (units in mm)
offset_initial = offset #5 #3.5
start_delay = 0 #(units in mm)

gcode_txt_imported = 'TESTGenerate_Checkerboard_SwitchingNozzle_gcode.txt' #"1DGC_Generate_Gradient_M1_gcode.txt" #"1DGC_Generate_Checkerboard_SwitchingNozzle_gcode.txt" #"1DGC_Generate_Checkerboard_M1.txt"
final_gcode_txt_export = gcode_txt_imported.replace('gcode', 'aerotech_test') #'230831_1mm_32_70_Gradient_diamond_lattice_aerotech.txt'
...
def generate_gcode(final_gcode_txt_export, accel, Z_var, z_o, feed, Sum_G_command_dict, Sum_var_dict, Sum_coord_dict, resync_trigger_distance_dict):
    ...
    with open(final_gcode_txt_export, "w") as f:
        with open(intro_gcode, "r") as g:
            for line in g:
                line = line.replace('{Z_var}', Z_var).replace('{ramprate}', str(accel)).replace('{feed}',str(feed)).replace('{Z_start}', str(Z_start))
                f.write(line)
            g.close()
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        with conn:
            ...
            while (i < len(command_dict_final)):
                ...
                if resync_type != False and len(data) > 0:
