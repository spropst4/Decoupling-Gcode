...
gcode_txt_imported = '231027_GradientBand_A_ThickBand_NG_gcode.txt'#'231027_GradientInfillV5_Rectangle_A_EdgeIncrease+NoGrad_gcode.txt' #"1DGC_Generate_Gradient_M1_gcode.txt" #"1DGC_Generate_Checkerboard_SwitchingNozzle_gcode.txt" #"1DGC_Generate_Checkerboard_M1.txt"
...
def generate_gcode(final_gcode_txt_export, accel, Z_var, z_o, feed, Sum_G_command_dict, Sum_var_dict, Sum_coord_dict, resync_trigger_distance_dict):
    ...
    resync_PING = ('\n\rFILEWRITE $hFile1, TIMER(0, PERFORMANCE)\n\rFILEWRITE $hFile2, TIMER(0, PERFORMANCE)' )

    with open(final_gcode_txt_export, "w") as f:
        f.write("DVAR $hFile1\n\rDVAR $hFile2\n\rDVAR $timer\n\r")
        f.write('G71 \nG76 \nG91	;G90 = absolute, G91 = relative \nG68 \nRAMP TYPE LINEAR X Y \nRAMP RATE ' +str(accel)+ '\n\rVELOCITY OFF\n\r')

        f.write("ENABLE X Y " + Z_var + " " + Z_var2 + "\n")
        if home == True:
            f.write("HOME X Y " + Z_var + " " + Z_var2 + "\r\n")
            f.write("'G90\n'G0 X0 Y0 " + Z_var + "0 " + Z_var2 + "0\n'G91\n\r")
        else:
            f.write("'HOME X Y " + Z_var + " " + Z_var2  + "\r\n")
            f.write("G90\nG0 X0 Y0 " + Z_var + "0 " + Z_var2 + "0\nG91\n\r")
        ...
        f.write("G0 X150 Y50 \n")
        f.write("G0 " + Z_var + str(z_o) + " " + Z_var2 + str(z_o)+ "\n")
        f.write("G1 F" + str(feed) + "\n\r")

        f.write("\n\rFILECLOSE")
        f.write('\n$hFile1 = FILEOPEN "COM' + str(port_python) + '", 2')
        f.write('\nCOMMINIT $hFile1, "baud=115200 parity=N data=8 stop=1"')
        f.write('\nCOMMSETTIMEOUT $hFile1, -1, -1, 1000')

        f.write('\n$hFile2 = FILEOPEN "COM' + str(port_python2) + '", 2')
        f.write('\nCOMMINIT $hFile2, "baud=115200 parity=N data=8 stop=1"')
        f.write('\nCOMMSETTIMEOUT $hFile2, -1, -1, 1000')

        f.write('\n\rFILEWRITE $hFile1, "start"')
        f.write('\n\rFILEWRITE $hFile2, "start"')
        ...
        for i in range(len(Sum_coord_dict)):
            ...
            for j in range(len(Sum_coord_dict[i])):
                ...
                if variable != 0 :
                    ...
                    if variable == Z_var:
                        coordinates += str(Z_var2) + str(dist) + " "
        ...
        f.write('\n\rG0 ' + str(Z_var) + '30 ' + str(Z_var2) + '30')
        f.write('\n\rFILECLOSE\nM02') # $hFile\nM02')
