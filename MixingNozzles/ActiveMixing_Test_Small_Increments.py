...
##INPUTS#############################################################################################################
export_file = "231030_AvtiveMixingGradient_gcode.txt"
save_path = 'C:\\Users\\MuellerLab_HPC\\PycharmProjects\\Gcode_generator\\SPropst_Decoupling'

# Desired XYZ motion
x_incr = 30 #distance increments to change pressure
x_total = 30 # distance for decr to incr gradient sections (incr or decr) is x_total/2
y_spacing = 1
total_y_dist = 25
num_fil_gradient = 1 # number of filaments to spread gradient over
z = 0.65 #1  # 0.58
Z_var = "Z"

pressure_start = [24, 24] #[25, 25]
pressure_end = [24, 24]

total_num_pressure_changes = int(x_total / x_incr)

pressure_change_incr1 = 1/num_fil_gradient*((pressure_end[0]-pressure_start[0])/(total_num_pressure_changes))
pressure_change_decr2 = 1/num_fil_gradient*((pressure_start[1]-pressure_end[1])/(total_num_pressure_changes))
######################################################################################################################
