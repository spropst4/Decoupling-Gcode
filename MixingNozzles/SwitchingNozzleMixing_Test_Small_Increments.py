...
##INPUTS#############################################################################################################
export_file = "231102_SwitchingMixingGradient_gcode.txt"
save_path = 'C:\\Users\\MuellerLab_HPC\\PycharmProjects\\Gcode_generator\\SPropst_Decoupling'

# Desired XYZ motion
x_incr = 25 #distance increments to change pressure
x_total = 25 # distance for decr to incr gradient sections (incr or decr) is x_total/2
y_spacing = 1
total_y_dist = 5
num_fil_gradient = 10 # number of filaments to spread gradient over
z = 0.65 #1  # 0.58
Z_var = "Z"

pressure_start = [28, 0]#[[28,0], [27,23], [26,24], [25.5, 25.5], [24, 26], [23,27], [0, 28]]
pressure_end = [28, 0] #

total_num_pressure_changes = int(x_total / x_incr)

pressure_change1 = 1 / num_fil_gradient * ((pressure_end[0] - pressure_start[0]) / (total_num_pressure_changes))
pressure_change2 = 1 / num_fil_gradient * ((pressure_end[1] - pressure_start[1]) / (total_num_pressure_changes))
...
with open(completeName, "w") as f:
    ...
    for i in range(int(total_y_dist)):
        ...
        for j in range(total_num_pressure_changes):
            ...
            pressure1 += pressure_change1
            pressure2 += pressure_change2
