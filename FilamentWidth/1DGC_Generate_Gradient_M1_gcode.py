...
##INPUTS#############################################################################################################
export_file = "1DGC_Generate_Gradient_M1_gcode.txt"
# Desired XYZ motion
x_incr = .25
x_total = 50
z = 0.58 #1  # 0.58
...
setpress1 = str('\n\r' + com[0] +'.write(' + str(setpress(pressure_start[0])) + ')') # material 1
...
with open(export_file, "w") as f:
    ...
    for i in range(int(x_total / x_incr)+1):
        if i == 0:
            f.write(str('\n\r' + com[0] + '.write(' + str(setpress(pressure_start[0] + add)) + ')'))  # material 1
            add += pressure_incr
            f.write(toggleON)
            f.write('\n\rG1 Y10')
