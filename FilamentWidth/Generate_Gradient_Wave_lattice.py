...
def generateCircle(num_sections_per_half, r_start, filament_width, export_gcode_txt, setpress_list, toggle_ON, toggle_OFF, pressure_start_original, pressure_end_original, num_segments, segment):
    with open(export_gcode_txt, 'w') as f:
        ...
        num_sections_per_half = int(num_sections_per_third)
        theta_between_moves = (2*np.pi/3) / num_sections_per_half
        print(num_sections_per_half)
        ...
        for layers in range(1):
            for lines in range(1):
                ...
                for segment in range(1):
                    if (segment + 1)%2 != 0:
                        circ_direction = 'G3' # counter clockwise
                    else:
                        circ_direction = 'G2'
                    ...
                    x_prev = r*np.sqrt(3)/2 #r
                    y_prev = 0.5*r #0

                    for c in range(num_sections_per_half):
                        x = round(r * np.cos(theta_for_xy), 5)
                        y = round(r * np.sin(theta_for_xy),5)
                        ...
                        i = round(-r*np.cos(theta_for_ij),5)
                        j = round(-r*np.sin(theta_for_ij),5)
                ...
                f.write('\n\rG0 Y'+str(r_start*2.5))
                f.write('\n\rG0 X' + str((r_start * 2)*num_segments))
...
circumference_one_third = 2/3 * np.pi*radius

num_sections_per_half = (circumference_half)/arc_length_incr

num_sections_per_third = circumference_one_third/arc_length_incr
