...
def Gradient_line_segmentation(input_line, segments, pressure_range, pressure_gradient_region,valveON, valveOFF,):
    ...
    pressure_incr = 0
    if pressure_gradient_region > 0:
        if pressure_gradient_region >= num_segments//2:
            pressure_gradient_region = num_segments//2 - even_extra
        pressure_incr = (pressure_range[1] - pressure_range[0]) / (pressure_gradient_region/segment_len)
    ...
    if len(input_dist) == 1: # horizontal or vertical lines
        for i in range(num_segments):
            ...
            if i == 0:
                ...
                pressure = pressure_range[1]
            elif i <= pressure_gradient_region/segment_len:
                pressure -= pressure_incr
                pressure_list[i] = pressure
                valve_list[i] = [valveOFF, valveON]

            elif i >= num_segments - pressure_gradient_region/segment_len:
                pressure += pressure_incr
                pressure_list[i] = pressure
    ...
    for layer in range(num_layers):
        ...
        for fil in range(num_filaments):
            ...
            if (layer + 1)%2 != 0: # odd layer
                ...
                input_line = [['X'], [x_dist]]
                Gradient_line_segmentation(input_line, segments, pressure_range_filament, pressure_gradient_region, valveON, valveOFF,)
...
if __name__ == '__main__':
    ### File names
    export_file = '230906_GradientInfillV4_gcode.txt'
