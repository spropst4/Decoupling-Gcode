...
def triangle2M(export_file_txt, outer_length,inner_length, setpress_list, toggle_ON, toggle_OFF):
    with open(export_file_txt, "w") as f:
        ...
        f.write('\n\rG1 X' + str(x / 1.5) + ' Y' + str(y / 1.5))

        f.write(toggle_ON[0])
        f.write(toggle_OFF[1])
        color_flag_ON = 0
        f.write('\n\rG1 X' + str(x / 4.5) + ' Y' + str(y / 4.5))

        while length > inner_length:
            if length != outer_length:
                x = (length-1)/2
                y = x*slope
            f.write('\n\rG1 X' + str(x/2) + ' Y' + str(y/2))
...
outer_length = 30
inner_length = 2
export_file_txt = 'SimpleShapes_Triangle_2Materials_V3_gcode.txt'
