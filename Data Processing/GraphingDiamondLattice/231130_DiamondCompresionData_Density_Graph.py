...
def find_max_list_and_extend_n(list_of_lists):
    ...
    for elem in list_of_lists: # appends zeros to end of data sets that are too short/failed earlier
        elem.extend([math.nan for i in range(max_len - len(elem))])
...
'''Crops the data to the shortest strain value'''
...
gradient_avg_stress = pd.DataFrame(gradient_stress_list).mean(axis=0).to_list()   #find_average_curve(gradient_stress_list, 10000)
gradient_avg_strain = pd.DataFrame(gradient_strain_list).mean(axis=0).to_list()  #find_average_curve(gradient_strain_list, 0.7)

gradient_std_stress= pd.DataFrame(gradient_stress_list).std(axis=0).to_list()

gradient_std_stress_upper= [gradient_std_stress[i] + gradient_avg_stress[i] for i in range(len(gradient_std_stress))]
gradient_std_stress_lower= [-gradient_std_stress[i] + gradient_avg_stress[i] for i in range(len(gradient_std_stress))]
print('Average G STD = ', np.average(gradient_std_stress))

NOgradient_avg_stress = pd.DataFrame(NOgradient_stress_list).mean(axis=0).to_list()
NOgradient_avg_strain = pd.DataFrame(NOgradient_strain_list).mean(axis=0).to_list()
NOgradient_std_stress = pd.DataFrame(NOgradient_stress_list).std(axis=0).to_list()

NOgradient_std_stress_upper = [NOgradient_std_stress[i] + NOgradient_avg_stress[i] for i in range(len(NOgradient_std_stress))]
NOgradient_std_stress_lower = [-NOgradient_std_stress[i] + NOgradient_avg_stress[i] for i in range(len(NOgradient_std_stress))]
print('Average NG STD = ', np.average(NOgradient_std_stress))
...
fig1 = plt.figure()
plt.plot(NOgradient_avg_strain, NOgradient_avg_stress, color = NG_color, label ='No Gradient') # plots average as a line
plt.fill_between(NOgradient_avg_strain, NOgradient_std_stress_lower, NOgradient_std_stress_upper, alpha = 0.15, color =NG_color) # plots std as an shaded area

plt.plot(gradient_avg_strain, gradient_avg_stress, color ='c', label ='Gradient')
plt.fill_between(gradient_avg_strain, gradient_std_stress_lower, gradient_std_stress_upper, alpha = 0.15, color =gradient_color)
